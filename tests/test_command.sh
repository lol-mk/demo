...
LOCAL="$PYTHON demo/local.py"
SERVER="$PYTHON demo/server.py"

assert "$LOCAL --version 2>&1 | grep demo | awk -F\" \" '{print \$1}'" "demo"
assert "$SERVER --version 2>&1 | grep demo | awk -F\" \" '{print \$1}'" "demo"
